...
public class Codec {
    ...
    public string serialize(TreeNode root, int level) {
        if(root == null) return "";
        var left = serialize(root.left, level + 1);
        var right = serialize(root.right, level + 1);
        return $"{level}|{root.val} {left} {right}";
    }
    ...
    private TreeNode deserialize(List<int[]> vals, int l, int r, int level) {
        ...
        var lInd = -1;
        var rInd = -1;
        if(l + 1 < vals.Count && vals[l+1][0] == nLevel && root.val > vals[l+1][1]) {
            lInd = l+1;
        }
        for(int i = l+1; i < r; ++i) {
            if(rInd < 0 && vals[i][0] == nLevel && root.val < vals[i][1]) {
                rInd = i;
                break;
            }
        }
